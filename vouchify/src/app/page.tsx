...
/** The hero used to carry this as a paragraph. Three places a stand goes, one
 *  line each, in the order an owner walks their own shop. */
const placementStrip = [
  {
    label: "Counter",
    headline: "While they pay",
    line: "The moment they are still standing in front of you and still pleased.",
  },
  {
    label: "Register",
    headline: "Next to the terminal",
    line: "Their phone is already out and already in their hand.",
  },
  {
    label: "Waiting area",
    headline: "Before they leave",
    line: "Idle minutes are the cheapest attention you will ever get.",
  },
] as const;
  ...
  return (
    <main id="main">
      ...
      <section className="bg-paper text-ink">
        <Container className="pt-16 pb-14 md:pt-24 md:pb-20">
          ...
          <Grid className="items-center gap-y-10 md:gap-y-14">
            <div className="order-2 col-span-4 md:order-1 md:col-span-7">
              {/* One eyebrow, one headline, one subhead, the buttons. Nothing
                  else above the fold. The paragraph that used to live here is
                  now the placement strip below. */}
              <p className="inline-flex items-center gap-2 rounded-pill border border-line bg-panel py-1.5 pr-3.5 pl-3">
                <span
                  aria-hidden="true"
                  className="size-1.5 rounded-pill bg-brass"
                />
                <span className="font-mono text-2xs font-medium uppercase tracking-[0.12em] text-ink-soft">
                  Model VC-01 &middot; Black stand
                </span>
              </p>
        ...
        {/* ---------------------------------------------------------------
            PLACEMENTS. A hairline, then the hero's old paragraph broken into
            the three places a stand actually goes, plus the one place it
            cannot. Quiet on purpose: mono label, one bold word, one line.
        ---------------------------------------------------------------- */}
        <div className="border-t border-line">
          <Container className="py-10 md:py-12">
            <dl className="grid grid-cols-1 gap-x-8 gap-y-8 sm:grid-cols-3">
              {placementStrip.map((spot) => (
                <div key={spot.label}>
                  ...
                  <dd className="mt-2.5">
                    ...
                    <span className="mt-1.5 block max-w-[30ch] text-sm text-ink-soft">
                      {spot.line}
                    </span>
                  </dd>
                </div>
            ...
            <dl className="mt-9 flex flex-wrap items-baseline gap-x-10 gap-y-3 border-t border-line pt-6">
              ...
              {three && threeEconomics ? (
                <div className="flex items-baseline gap-3">
                  <dt className="font-mono text-2xs font-medium uppercase tracking-[0.12em] text-ink-soft">
                    Three stands,{" "}
                    <span data-numeric>
                      {formatMoney(threeEconomics.perUnitCents)} each
                    </span>
                  </dt>
                  <dd
                    data-numeric
        ...
        <Grid className="gap-y-8">
          <div className="col-span-4 md:col-span-12 lg:col-span-4 lg:row-start-1">
            ...
            <h2 className="mt-4 text-2xl md:text-3xl">
              Two things. That is all we make.
            </h2>
          </div>
          ...
          <div className="col-span-4 md:col-span-12 lg:col-span-4 lg:col-start-1 lg:row-start-2 lg:self-end lg:pb-2">
            <p className="max-w-prose text-base text-warm-700">
              A black acrylic stand for the counter, and a blue and white square
              plate for the window and the places a stand will not sit. Both
              hold the same chip, both carry a printed QR code, and both point
              at the one link you control.
            </p>
            <p className="mt-4 max-w-prose text-sm text-warm-600">
              Drawn to the same scale, so the two sizes are honest against each
              other.
            </p>
          </div>
        </Grid>
        ...
        <Grid className="mt-12 gap-y-8 md:mt-16">
          <div className="col-span-4 md:col-span-4 md:col-start-6">
            <p className="text-lg text-warm-700">
              Nobody walks out of a good haircut planning to write a review.
              They mean it when they say it was great, and then they are back in
              the car thinking about lunch.
            </p>
          </div>
          <div className="col-span-4 md:col-span-3 md:col-start-10">
            <p className="text-base text-warm-700">
              The ask only works while they are still standing in front of you.
              A stand on the counter makes it at that moment, without your staff
              saying a word.
            </p>
          </div>
        </Grid>
      ...
      <Section tone="warm" bordered>
        <Grid className="items-center gap-y-12">
          ...
          <div className="col-span-4 md:col-span-6 md:col-start-7">
            <h2 className="text-2xl md:text-3xl">
              A piece of acrylic that does one job.
            </h2>
            <p className="mt-5 max-w-prose text-base text-warm-700">
              There is no clever part. A chip inside holds a web address. A
              phone held against it opens that address. The QR code on the face
              covers the phones that will not tap.
            </p>
        ...
        <Grid className="gap-y-12">
          <div className="col-span-4 md:col-span-4">
            ...
            <p className="mt-5 text-base text-warm-700">
              Every stand covers exactly the customers who walk past it. That is
              why shops buy more than one. Not the price break, the coverage.
            </p>
            ...
            <ol className="mt-9 border-t border-warm-300">
              {placements.slice(0, 4).map((placement) => {
                ...
                return (
                  <li
                    key={placement.n}
                    className="flex gap-5 border-b border-warm-300 py-4"
                  >
                    <span
                      data-numeric
                      className={
                    ...
                    <span className="min-w-0">
                      ...
                      <span className="mt-0.5 block text-sm text-warm-700">
                        {placement.note}
                      </span>
                    </span>
                  </li>
            ...
            <dl className="mt-8 border-t border-warm-800">
              {coreProduct.ownership.map((term) => (
                ...
                >
                  ...
                  <dd className="flex-1 text-sm font-medium text-warm-300">
                    {term.value}
                  </dd>
                </div>
              ))}
          ...
          <ol className="col-span-4 grid grid-cols-1 gap-px overflow-hidden rounded-md border border-warm-300 bg-warm-300 sm:grid-cols-3 md:col-span-8 md:col-start-5">
            {setupSteps.map((step) => (
              ...
              >
                ...
                <span className="mt-2 block text-sm text-warm-700">
                  {step.body}
                </span>
              </Reveal>
            ))}
        ...
        <Grid className="items-center gap-y-6">
          <div className="col-span-4 md:col-span-7">
            ...
            <p className="mt-4 max-w-prose text-base text-warm-300">
              Programmed to your review link before it ships, and tapped on both
              an iPhone and an Android handset as part of packing.
            </p>
          </div>
