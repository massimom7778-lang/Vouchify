...
This repository holds **two** separate applications side by side — `vouchify/` and
`meridian-concours/`. There is no app at the repository root, so Vercel must be
told which one to build:

1. **Project → Settings → Build and Deployment → Root Directory: `vouchify`.**
...
2. **Project → Settings → Git → Production Branch:** set it to the branch that
   actually contains `vouchify/`. Vercel deploys the production branch, which
...
Chips are encoded once with a short Vouchify address (`/r/K3M7QX2`) and never
